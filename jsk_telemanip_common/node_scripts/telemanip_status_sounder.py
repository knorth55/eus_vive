...
from jsk_telemanip_msgs.msg import TelemanipStatusArray
from sound_play.msg import SoundRequest


class TelemanipStatusSounder(ConnectionBasedTransport):

    def __init__(self):
        super(TelemanipStatusSounder, self).__init__()
    ...
    def subscribe(self):
        self.status_sub = rospy.Subscriber(
            '~input/status', TelemanipStatusArray, self._status_cb)
    ...
    def _timer_cb(self, event):
        ...
        if larm_start or rarm_start:
            ...
            sound_msg.arg = os.path.join(
                self.rospack.get_path('jsk_telemanip_common'),
                'sounds/start.wav')
...
if __name__ == '__main__':
    rospy.init_node('telemanip_status_sounder')
    app = TelemanipStatusSounder()
    rospy.spin()
