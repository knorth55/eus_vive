...
class TABLIS_Configurator:
    Groups = {'rleg': ['RLEG_JOINT0', 'RLEG_JOINT1', 'RLEG_JOINT2', 'RLEG_JOINT3', 'RLEG_JOINT4', 'RLEG_JOINT5'],
    ...
    rh_svc = rtm.findService(findComp("RobotHardware0"),"RobotHardwareService","RobotHardwareService","service0")._narrow(OpenHRP.RobotHardwareService)
    seq_svc = rtm.findService(findComp("seq"),"SequencePlayerService","SequencePlayerService","service0")._narrow(OpenHRP.SequencePlayerService)
    sh_svc = rtm.findService(findComp("sh"),"StateHolderService","StateHolderService","service0")._narrow(OpenHRP.StateHolderService)
    hc_svc = rtm.findService(findComp("hc"),"SimpleHapticsControllerService","SimpleHapticsControllerService","service0")._narrow(OpenHRP.SimpleHapticsControllerService)
    log_svc = findComp("log").service("service0")._narrow(OpenHRP.DataLoggerService)
    ...
    def setHcParametersJAXON(self):
        ...
        hcp.max_torque = [20,80,80,80,20,20, 20,80,80,80,20,20, 40, 30,30,20,20,12,8,8,6, 30,30,20,20,12,8,8,6]
    ...
    def init(self):
        for j in self.Groups["rleg"] + self.Groups["lleg"] + self.Groups["torso"] + self.Groups["rarm"] + self.Groups["larm"]:
