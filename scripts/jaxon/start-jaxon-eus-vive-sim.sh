...
new-window connect_leader "${SOURCE_SCRIPT} && ${TABLIS_MASTER} && roslaunch eus_vive tablis_bridge_leader.launch filtered:=true"
new-window connect_follower "${SOURCE_SCRIPT} && ${JAXON_MASTER} && roslaunch eus_vive tablis_bridge_follower.launch"
new-window wbms "${SOURCE_SCRIPT} && ${JAXON_MASTER} && roslaunch biped_wbms_jaxon wbms.launch"
new-window eus_vive "${SOURCE_SCRIPT} && ${JAXON_MASTER} && roslaunch eus_vive jaxon_tablis_choreonoid.launch"
